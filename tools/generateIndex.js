        ...
        if (fullPath.match) {
            if (
                ...
                if (directoryList.length) {
                    ...
                    fs.existsSync(indexLocation) &&
                        fs.unlinkSync(indexLocation);
...
function getDirectoryList(dir) {
    ...
    fs.readdirSync(dir).forEach((file) => {
        ...
        if (
            fs.lstatSync(fullPath).isDirectory() &&
            !fs.readdirSync(fullPath).length
        )
            return;
        ...
        if (splitName.length > 2) {
            ...
            console.log(
                `[generateIndex] Skipping file: ${file} please manually check if this is needed`,
            );
...
if (watch) {
    // eslint-disable-next-line no-console
    console.log(
        "[generateIndex] watching for file changes, don't rewrite the index files... we will do it for you!",
    );
