...
export class MaskedFrame extends Container {
    ...
    constructor(public target: Container, options: MaskedFrameOptions = {}) {
        ...
        const isCircle =
            ...
            options.radius >= target.width / 2;

        if (
            options.borderColor !== undefined &&
            options.borderWidth !== undefined
        ) {
            this.border = isCircle
                ? this.getCircleLine(options.borderColor, options.borderWidth)
                : this.getBoxLine(
                      options.borderColor,
                      options.borderWidth,
                      options.radius,
    ...
    private getCircleLine(color: number, w: number): Graphics {
        ...
        const box = new Graphics()
            .beginFill(color)
            .drawCircle(middle + offset, middle + offset, middle + offset);
